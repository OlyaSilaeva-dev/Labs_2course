...
void log_(FILE* file, log_msg* msg, int* seek, my_time* tm)
{
    ...
    *seek += fprintf(file, "Application receipt, branch ID - %s, application ID: %d\n", msg->ident_otd, msg->ident_app);

    break;
    
    case REQUEST_HANDLING_STARTED:
    *seek += fprintf(file, "%s", "[REQUEST_HANDLING_STARTED]: ");
    *seek += fprintf(file, "The beginning of the application processing. Application ID - %d, operator - %s\n", msg->ident_app, msg->name_op);
    break;

    case REQUEST_HANDLING_FINISHED:
    *seek += fprintf(file, "%s", "[REQUEST_HANDLING_FINISHED]: ");
    *seek += fprintf(file, "The application has been processed. ID %d, operator %s, execution time - %d min\n", msg->ident_app, msg->name_op, msg->time_treatment);
    break;

    case DEPARTMENT_OVERLOADED:
    *seek += fprintf(file, "%s", "[DEPARTMENT_OVERLOADED] ");
    *seek += fprintf(file, "branch %s it's overloaded. The overload occurred after the request with the ID %d. ", msg->ident_otd, msg->ident_app);
    if (msg->name_op)
    {
        *seek += fprintf(file, "branch %s submitted its applications to the department %s\n", msg->ident_otd, msg->name_op);
    }
    else *seek += fprintf(file, "branch %s cannot transfer applications to any branch\n", msg->ident_otd);
    break;
